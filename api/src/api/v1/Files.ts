...
import { createWriteStream, appendFileSync, createReadStream, existsSync, mkdirSync, readdirSync, renameSync, rmSync, statSync, writeFileSync } from 'fs'
...
export class Files {
  ...
  public static async download(req: Request, res: Response, files: files[], onlyHeaders?: boolean): Promise<any> {
    ...
    for (const file of files) {
      ...
      const getData = async () => {
        try {
          await req.tg.downloadMedia(chat['messages'][0].media, (progress) => {
            downloaded = progress
            if (cancel) {
