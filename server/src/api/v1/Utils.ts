...
export class Utils {
  ...
  public async maintenance(_: Request, res: Response): Promise<any> {
    return res.send(
      {
        maintenance: ['1', 'true', 'yes']
          .includes(process.env.IS_MAINTENANCE)
      }
    )
  }
    ...
    return res.send(
      {
        ipinfo: {
          ...
          ...lookup(req.headers['cf-connecting-ip'] as string || req.ip)
        }
      }
    )
  }
  ...
  public async simpleAnalytics(_: Request, res: Response): Promise<any> {
    return res.send(
      {
        analytics: await Redis.connect().getFromCacheFirst(
          'simpleAnalytics',
          async () => {
            return {
