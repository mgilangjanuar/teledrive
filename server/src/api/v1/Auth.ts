...
export class Auth {
  ...
  public async reSendCode(req: Request, res: Response): Promise<any> {
    ...
    if (!phoneNumber || !phoneCodeHash) {
      throw {
        status: 400,
        body: {
          error: 'Phone number and phone code hash are required'
        }
      }
    }
    ...
    const { phoneCodeHash: newPhoneCodeHash } = await req.tg.invoke(
  ...
  public async login(req: Request, res: Response): Promise<any> {
    ...
    if ((!phoneNumber || !phoneCode || !phoneCodeHash) && !password) {
      // TODO Isn't that condition unnecessary?
      if (!password) {
        throw {
          status: 400,
          body: {
    ...
    if (password) {
      ...
      data.newAlgo['salt1'] = Buffer.concat(
        [
          data.newAlgo['salt1'],
      ...
      signInRequest = new Api.auth.CheckPassword(
        { password: await computeCheck(data, password) }
      )
    } else {
      signInRequest = new Api.auth.SignIn(
        { phoneNumber, phoneCode, phoneCodeHash }
      )
    }

    const signIn = await req.tg.invoke(signInRequest)
    ...
    if (!userAuth) {
      throw {
        status: 400,
        body: {
          error: 'User not found or authorized'
        }
      }
    }
    ...
    res
      ...
      .cookie(
        ...
        auth.refreshToken,
        {
          maxAge: 3.154e+10,  // TODO make that number into a constant, somewhere?
    ...
    Files.createQueryBuilder('files')
      .where(
        'user_id = :user_id and signed_key is not null',
        { user_id: user.id }
      )
      .getMany()
  ...
  public async refreshToken(req: Request, res: Response): Promise<any> {
    ...
    // TODO make { session: string } a type alias?
    ...
    try {
      ...
      const user = await Users.findOne(
        { tg_id: await req.tg.getMe()['id'].toString() }
      )
      if (!user) {
        throw {
          status: 404,
  ...
  public async qrCodeSignIn(req: Request, res: Response): Promise<any> {
    ...
    if (password && sessionString) {
      ...
      const signIn = await req.tg.invoke(
      ...
      if (!userAuth) {
        throw {
          status: 400,
          body: {
            error: 'User not found/authorized'
          }
        }
      }
    ...
    // handle the second call for export login token, result case: success,
    // need to migrate to other dc, or 2fa
    await req.tg.connect()
    ...
    try {
      ...
      const buildResponse = (
        data: Record<string, any> & { user?: { id: string } }
      ) => {
        const session = req.tg.session.save()
      ...
      if (data instanceof Api.auth.LoginTokenMigrateTo) {
        ...
        const result = await req.tg.invoke(
          new Api.auth.ImportLoginToken({ token: data.token })
        )
        ...
        data instanceof Api.auth.LoginTokenSuccess &&
        data.authorization instanceof Api.auth.Authorization
      ) {
        // handle if success
