    ...
    <Layout.Content className="container">
      <Row>
        <Col lg={{ span: 18, offset: 3 }} md={{ span: 20, offset: 2 }} span={24}>
          ...
          <Typography.Paragraph>
            A: For authentication and authorization flow, we using <a href="https://core.telegram.org/api/auth" target="_blank">this flow</a>. First, we need a valid phone number and hit <a href="https://core.telegram.org/method/auth.sendCode" target="_blank">send code</a> method to the Telegram API, then <a href="https://core.telegram.org/method/auth.signIn" target="_blank">sign in</a> with the 5 digits code that is sent to the user's Telegram account.
            If the code has expired, we use <a href="https://core.telegram.org/method/auth.resendCode" target="_blank">resend code</a> method. After that, we got the session key, TeleDrive will wrap the session with the <a href="https://jwt.io/" target="_blank">JWT</a> method to make you securely access other private endpoints, like upload, download, get file lists, etc. And we do not store your session in any storage or databases.
          </Typography.Paragraph>
          <Typography.Paragraph>
            For upload and download flow, we using the <a href="https://core.telegram.org/method/upload.getFile" target="_blank">get file</a> and <a href="https://core.telegram.org/method/upload.saveBigFilePart" target="_blank">save big file part</a> methods. For every file a user uploads, it gets uploaded in chunks and passed to Telegram. So, you can't reload or close the browser if it still uploading. All files that you upload will be
          ...
          <Typography.Paragraph>
            A: You can report it to this link: <a href="https://teledriveapp.com/contact?intent=report" target="_blank">https://teledriveapp.com/contact?intent=report</a>. All the reports from users will be received to the personal Telegram account of the creator.
          </Typography.Paragraph>
          <Typography.Title level={5}>
            Q: Will you have ads in my dashboard? Or sell my data?
          </Typography.Title>
          <Typography.Paragraph>
            A: We’ll not sell any data for now and forever.
          </Typography.Paragraph>
          <Typography.Paragraph>
            But, for ads, we’ll add them to the homepage and not in the user’s dashboard. So, users are not distracted by ads or tracked by the files they upload.
          </Typography.Paragraph>
          <Typography.Title level={5}>
